...
echo -e "Test #1 - base"
valgrind --error-exitcode=1 --leak-check=full --quiet ./inode_info ext2.img $INODE_BASE
valgrind --error-exitcode=1 --leak-check=full --quiet ./inode_print ext2.img $INODE_BASE >base.txt

PARSED_BASE_SUM=$(sha512sum base.txt | awk '{print $1}')
if [ "$SUM_BASE" == "$PARSED_BASE_SUM" ]; then
  echo -e "Control sums equal"
else
  echo -e "Control sums differ"
  exit 1
fi
...
valgrind --error-exitcode=1 --leak-check=full --quiet ./inode_print ext2.img $INODE_DIR | ./print_dir >parsed_dir.txt
cat parsed_dir.txt
...
valgrind --error-exitcode=1 --leak-check=full --quiet ./inode_info "$LOOP_DEV" "$INODE_BASE" >/dev/null
LOOP_SUM=$(./inode_print "$LOOP_DEV" "$INODE_BASE" | sha512sum | awk '{print $1}')

if [ "$SUM_BASE" == "$LOOP_SUM" ]; then
  echo -e "Successfully read loop"
else
  echo -e "Error while reading loop"
  exit 1
fi
